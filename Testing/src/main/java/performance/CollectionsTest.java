...
public class CollectionsTest {
	...
	public CollectionsTest(int entitiesCount) {
		this.entitiesCount = entitiesCount;
	}

	private void init() {
		collection = new ConcurrentHashMap<>(entitiesCount);
	...
	private void execute(BiFunction<Integer, Integer, Integer> operation) throws InterruptedException {
		...
		for (int i = 0; i < Starter.THREADS_COUNT; i++) {
			threads.add(new Thread(new CollectionFillerRunnable<>(collection, values, operation)));
		}
		watch.resume();
